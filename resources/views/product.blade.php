<?php /* @extends('layouts.master', ['id' => 'product-product'])
...
@section('content')
    ...
    <p>Цена: <b>{{ $skus->price }} {{ App\Services\CurrencyConversion::getCurrencySymbol() }}</b></p>
    ...
    <div>
        <div class="thumbnail" style="max-width: 25%; margin: 0 auto;">
            <img src="{{ Storage::url($skus->product->image) }}" alt="{{ $skus->product->__('name') }}">
        </div>
    </div>
    ...
    <div class="text-left">
        @if($skus->product->reviews->count() > 0)
            <p>Отзывы: {{ $skus->product->reviews->count() }}</p>
            @foreach($skus->product->reviews as $review)
                <div>
                    <p>{{ (new DateTime($review->created_at))->format('m-d-Y') }}
                        , {{ $review->author ?? 'nameless' }}</p>
                    <div style="word-break: break-word;">{{ $review->text }}</div>
    ...
    <div style="max-width: 50%; margin: 0 auto 0 0; text-align: left;">
        <p class="text-left">Оставьте свой отзыв:</p>
        <form action="{{ route('reviews.store', $skus->product) }}" method="post">
            @csrf
            <div class="form-group">
                @error('author')
                <div class="alert alert-danger">{{ $message }}</div>
